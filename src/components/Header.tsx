...
export default function Header() {
  ...
  return (
    <>
      ...
      <header
        className="fixed top-0 left-0 right-0 z-50 bg-white/95 dark:bg-[#1E1E1E]/95 backdrop-blur-sm h-16 md:h-16 px-6 border-b border-[#E0E0E0]/50 dark:border-[#404040]/50"
      ...
      >
        ...
        {/* Mobile Menu Panel */}
        {isMobileMenuOpen && (
          <div className="md:hidden fixed inset-0 bg-white dark:bg-[#1E1E1E] z-50 flex flex-col">
            {/* Mobile Header */}
            <div className="flex items-center justify-between h-16 px-6 border-b border-[#E0E0E0] dark:border-[#404040]">
              <div className="flex items-center space-x-3">
                <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-[#6366F1] to-[#8B5CF6] flex items-center justify-center overflow-hidden">
                  <img
                    src={logo}
                    alt="Logo"
                    className="w-full h-full object-contain"
              ...
              <button
                className="p-2 text-[#121212] dark:text-white focus:outline-none focus:ring-2 focus:ring-[#6366F1] focus:ring-offset-2 rounded-lg"
            ...
            <nav className="flex-1 px-6 py-6 space-y-4">
              {menuItems.map((item) => (
                <button
                  key={item.name}
                  onClick={() => scrollToSection(item.href)}
                  className="flex items-center w-full py-3 text-[#121212] dark:text-white opacity-90 hover:opacity-100 hover:text-[#6366F1] dark:hover:text-[#8B5CF6] transition-colors duration-150 font-medium text-base border-b border-[#E0E0E0] dark:border-[#404040] last:border-b-0"
                >
                  {item.name}
                </button>
              ))}
